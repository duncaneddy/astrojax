...
# ///
"""Propagate active Celestrak satellites using SGP4 with multi-device parallelization.

Downloads GP elements for all active satellites from Celestrak, initializes SGP4
parameters using JIT-compiled vmap'd initialization, and propagates orbits across
all available JAX devices (GPUs or CPUs) using vmap + pmap.

Requires astrojax to be installed (``uv pip install -e .`` from the repo root).

Usage:
    python examples/celestrak_active_propagate.py [OPTIONS]

Examples:
    # Quick CPU smoke test
    python examples/celestrak_active_propagate.py --timestep 3600 --duration 0.01 --batch-size 4

    # Full 7-day propagation at 60s timestep
    python examples/celestrak_active_propagate.py --timestep 60 --duration 7.0
"""
...
def main(
    ...
    """Propagate active Celestrak satellites with SGP4 on multiple devices."""
    devices = jax.devices()
    ...
    print("\n── Stage 1: Downloading GP records from Celestrak ──")
